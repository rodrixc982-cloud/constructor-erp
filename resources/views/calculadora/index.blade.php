@extends('layouts.app')
@section('titulo', 'Calculadora Inteligente')
@section('breadcrumbs')<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li><li class="breadcrumb-item active">Calculadora</li>@endsection
...
const calcularMuro = () => calc('{{ route('calculadora.muro') }}', { area: document.getElementById('muro_area').value }, 'resultadoMuro');
const calcularConcreto = () => calc('{{ route('calculadora.concreto') }}', { volumen: document.getElementById('concreto_volumen').value }, 'resultadoConcreto');
const calcularAcero = () => calc('{{ route('calculadora.acero') }}', { volumen: document.getElementById('acero_volumen').value, elemento: document.getElementById('acero_elemento').value }, 'resultadoAcero');
const calcularPintura = () => calc('{{ route('calculadora.pintura') }}', { area: document.getElementById('pintura_area').value, manos: document.getElementById('pintura_manos').value }, 'resultadoPintura');
const calcularCeramica = () => calc('{{ route('calculadora.ceramica') }}', { area: document.getElementById('ceramica_area').value }, 'resultadoCeramica');
const calcularCompleta = () => calc('{{ route('calculadora.completa') }}', { area_m2: document.getElementById('cc_area').value, pisos: document.getElementById('cc_pisos').value, tipo: document.getElementById('cc_tipo').value }, 'resultadoCompleta');
</script>
@endpush
