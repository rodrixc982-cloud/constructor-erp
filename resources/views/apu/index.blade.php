...
@section('titulo', 'Análisis de Precios Unitarios (APU)')
...
$(document).ready(function() {
    ...
    const CATALOGO = {
        materiales: @json($materiales->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre, 'precio' => (float) $m->precio_venta])),
        manoObra: @json($manoObra->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre.' ('.$m->especialidad.')', 'costo' => (float) $m->costo])),
        equipos: @json($equipos->map(fn($e) => ['id' => $e->id, 'nombre' => $e->nombre, 'costo' => (float) $e->costo_alquiler_dia])),
        maquinarias: @json($maquinarias->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre, 'costo' => (float) $m->costo_dia])),
    };
